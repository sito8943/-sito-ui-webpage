...
function SitoGreet() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        if (!cancelled) setWeatherLoaded(false);
      }
    }
    run();
    return () => {
  ...
  // Observe elements with class `appear` inside this section to add a fade-in
  useEffect(() => {
    const root = document.getElementById("sito-greet");
    if (!root) return;
    const elements = root.querySelectorAll(".appear");

    // If IntersectionObserver is not available, just reveal immediately
    ...
    elements.forEach((el) => {
      // ensure initial hidden state and apply any inline delay
  ...
  return (
    ...
    >
      ...
      <header
        className="topInfo"
        aria-label={t("_pages:home.sitoGreet.headerAria")}
      >
        <p className="topText clock appear from-top" data-delay="0">
          {dateTimeFormatter.format(now)}
        </p>
        <p
          className="topText weather appear from-top"
          aria-live="polite"
          data-delay="120"
        >
      ...
      <main className="center" role="main">
        ...
        <div className="ctaRow appear" data-delay="220">
          <Link
            to="/sito-greet"
            className="ctaButton"
      ...
      <footer className="footer">
        ...
        <p className="footerCenter appear" data-delay="300">
        ...
        {/* Right side intentionally left empty (no settings button in preview) */}
